...
Create_ccRepair_Data <- function( Object_data= your_rds,
  ...
  print("Diff gene testing singlet samples")
  ...
  print(paste0(nrow(diff.exp.data)," genes will be adjusted in dyads. If this seems too low, adjust fold_change_baseline"))
  ...
  print("Picking most consistent genes to generate ccRepair ratios")
  ...
  gene.design$Class <- "Side1"
  save.gene.design<- gene.design
  ...
  gene.design$Class <- "Side1"
  save.gene.design<- rbind(save.gene.design,gene.design)
  ...
  print("making ccRepair ratio adjusted matrix")
 ...
 #####
  ...
  print("Searching for most likely dyads in sample, want to dispense with adjusted singlets")
  ...
  if(mean(cells.wanted$Score1)/mean(cells.wanted$Score2)>1.5|mean(cells.wanted$Score1)/mean(cells.wanted$Score2)<0.66){
    print(" Ratio far from 1, suggesting unequal mixing in dyads. Ideally, dyad_identity_min1 and 2 are set to reflect this skew. Does not affect calculation but may add too many non-dyads to adjusted matrix if threshold is too low for one of the scores")
    print(" If your dyad_identity_min1/dyad_identity_min2 ratio is already near the reported ratio, you're fine!")
    print(paste0("Reported identity ratio in putative dyads is Score1/Score2= ", round(mean(cells.wanted$Score1)/mean(cells.wanted$Score2),2) , " while input ratio is ", dyad_identity_min1/dyad_identity_min2 ))
  }
  ...
  print("Bringing nonadjusted cells and genes back in to join adjusted dyads in a final repaired matrix")
  ...
  if(write_output_data==T){
    print("writing outputs")
    
    write.table(repaired.ratio.df, gzfile(paste0(outdir, ExpName, ".ccRepaired.full.dataframe.txt.gz")),
                sep="\t", quote=F)
  
    write.table(cells.wanted2, gzfile(paste0( outdir, ExpName, ".ccRepaired.cell.metadata.txt.gz")),
                sep="\t", quote=F)
    
    write.table(save.gene.design, paste0( outdir, ExpName, ".ccRepaired.consistent.gene.data.txt.gz"),
                sep="\t", quote=F)
